...
#dm_diag_insdate is equivalent to first insulin prescription date
cohort <- cohort %>% mutate(survival_date = dm_diag_date + days(1095)) %>%
  mutate(survival_date = as.Date(ifelse(!is.na(dm_diag_insdate) & dm_diag_insdate < survival_date, dm_diag_insdate, survival_date))) %>% 
  ...
  mutate(outcome = ifelse(!is.na(dm_diag_insdate) & dm_diag_insdate == survival_date, 1, 0)) %>% mutate(survival_time = as.numeric(survival_date - dm_diag_date)) %>% 
  mutate(survival_time_years = survival_time/365)

#Check survival time distribution for those with insulin outcome
...
#lots of insulin initiation around diagnosis
...
KM_plot_PEI <- ggsurvplot(fit = fit, 
           ...
           ylab = "Cumulative incidence of insulin initiation (%)",
           ylim = c(0, 50),
           xlim = c(0,3),
           break.time.by = 1,
           font.ticks = 16,
           ...
           linetype = c("strata"),
           tables.theme = theme_cleantable(clip = "off"),
           axes.offset = FALSE)
...
pdf("SFig3_Time_to_insulin_3_year_by_PEI.pdf",width=12.5,height=9)
KM_plot_PEI
dev.off()
...
#Save
pdf.options(reset = TRUE, onefile = FALSE)
pdf("SFig4_Time_to_insulin_3_year_by_subtype.pdf",width=12.5,height=9)
KM_plot_subtype
dev.off()
